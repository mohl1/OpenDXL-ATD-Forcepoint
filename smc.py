...
def action(query):
   ...
   login_params = {"authenticationkey": auth}
   r = s.post(
       url + "login",
   ...
   r = s.get(url + "elements/ip_list", data=json.dumps(new_list), headers=h)
   r.raise_for_status()
   ...
   for entry in ip_lists:
       ...
       r = s.post(url + "elements/ip_list", data=json.dumps(new_list), headers=h)
       r.raise_for_status()
